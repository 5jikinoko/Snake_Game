...
bool move_snake(std::list<Point>& snake, std::vector<Point>& eggs, char field_status[height][width], int key){
    //移動後の先頭がhead2
    Point head1 = snake.front();
    Point head2 = head1;
    switch(key){
    ...
    if(bitten_egg==-1){
        ...
        snake.pop_back();
        field_status[(snake.back().y-1)/2][(snake.back().x-1)/2] = emp;
        mvprintw(snake.back().y, snake.back().x, " ");
        snake.pop_back();
...
{   
    ...
    for(int i=-(baby_snakesize+s)+2; i<baby_snakesize-s+2; ++i){
        snake.push_front(Point((width/2)-w+i, (height/2)-h+1));
